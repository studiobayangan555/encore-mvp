            ...
            <div key={r.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 24, marginBottom: 16 }}>
              ...
              {r.photos && r.photos.length > 0 && (
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' as const, marginBottom: 14 }}>
                  {r.photos.map((p, i) => <img key={i} src={p} alt="" style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: 8, border: '1px solid var(--border)' }} />)}
                </div>
              )}
      ...
      <div>
        ...
        <div style={{ paddingBottom: 22, borderBottom: '1px solid var(--border)', marginBottom: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            ...
            <a href={show.ticket_url || '#'} target="_blank" rel="noopener" style={{ fontFamily: 'Unbounded, sans-serif', fontWeight: 600, fontSize: 10, color: 'var(--accent)', textDecoration: 'none' }}>Full lineup →</a>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 12 }}>
            {[{ name: show.artist, role: 'Headliner' }, { name: 'Support TBA', role: 'Support' }].slice(0, LINEUP_LIMIT).map((act, i) => (
