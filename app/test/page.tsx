'use client'
import { useEffect, useState } from 'react'
import { createClient } from '@supabase/supabase-js'

export default function TestPage() {
  const [result, setResult] = useState<any>(null)
  const [envUrl, setEnvUrl] = useState('')
  const [envKey, setEnvKey] = useState('')

  useEffect(() => {
    const url = process.env.NEXT_PUBLIC_SUPABASE_URL || 'NOT SET'
    const key = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'NOT SET'
    setEnvUrl(url)
    setEnvKey(key.slice(0, 20) + '...')

    // Create a fresh client directly — no singleton, no auth state
    const supabase = createClient(url, key, {
      auth: { persistSession: false }
    })

    supabase.from('shows').select('id, artist, is_published').limit(3).then(({ data, error }) => {
      setResult({ data, error, timestamp: new Date().toISOString() })
    })
  }, [])

  return (
    <div style={{ padding: 40, fontFamily: 'monospace', background: '#080A0F', color: '#E8ECF4', minHeight: '100vh' }}>
      <h1 style={{ marginBottom: 20 }}>Supabase Connection Test</h1>
      <p style={{ marginBottom: 6 }}>URL: <strong style={{ color: '#E8FF47' }}>{envUrl}</strong></p>
      <p style={{ marginBottom: 20 }}>Key: <strong style={{ color: '#E8FF47' }}>{envKey}</strong></p>
      <pre style={{ background: '#0F1219', padding: 20, borderRadius: 8, fontSize: 13 }}>
        {JSON.stringify(result, null, 2)}
      </pre>
    </div>
  )
}
